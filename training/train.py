"""The unified training CLI: dispatches to every training mode by subcommand.

Four subcommands, from the simplest training path to the most complete one:

* ``autoencoder`` -- train the original from-scratch reconstruction
  autoencoder on one category (``training.legacy.main_autoencoder``).
* ``padim`` -- fit the original single-file PaDiM on one category
  (``training.legacy.main_padim``).
* ``sweep`` -- run the multi-method, multi-dataset benchmark sweep itself
  (``training.sweep.main``), writing to the SQLite results store.
* ``all`` -- the full six-stage study end to end: sweep, cost profiling,
  metrics backfill, leaderboard, ensembling, and deployment profiling. Each
  stage after the sweep is cheap and reads whatever the sweep has produced
  so far, so a run that dies partway can simply be relaunched.
...
Usage:
    ...
    python -m training.train sweep --models all --datasets all
    python -m training.train all
    python -m training.train all --quick
    python -m training.train <subcommand> --help
"""
...
from training import legacy, sweep
...
# -----------------------------------------------------------------------------
# `all`: the full six-stage study, in dependency order
# -----------------------------------------------------------------------------
#
# 1. sweep -- every method against every dataset configuration, in the
#    one-class, multi-class and few-shot regimes, across every requested
#    seed. This is the expensive stage and the only one that touches a GPU
#    for long. Crash-safe and resumable at the individual-run level, so
#    relaunching this exact command after an interruption (including a
#    reboot) picks up exactly where it stopped.
...
#    methods (a separate, multi-device comparison from the cost stage
#    above, which only measures the sweep's own device).
#
# Each stage runs as its own subprocess (`python -m training.<module>`) so a
# segfault or CUDA OOM in one cannot take this orchestrator down with it --
# the later stages still produce a report from whatever the sweep managed to
# finish. Each stage is independently resumable, so a run that dies partway
...
def main_all(argv: list[str] | None = None) -> None:
    """Parse arguments and run every requested stage of the full study."""
    parser = argparse.ArgumentParser(description=main_all.__doc__)
...
_SUBCOMMANDS = {
    "autoencoder": legacy.main_autoencoder,
    "padim": legacy.main_padim,
    "sweep": sweep.main,
    "all": main_all,
}
...
def main(argv: list[str] | None = None) -> None:
    """Dispatch to one of the four training modes by subcommand.

    Usage:
        python -m training.train <autoencoder|padim|sweep|all> [options]
        python -m training.train <subcommand> --help
    """
