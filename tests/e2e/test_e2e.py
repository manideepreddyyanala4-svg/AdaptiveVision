...
# Where the MVTec bottle category lives locally. Override with the
# ADAPTIVEVISION_DATA_ROOT env var; otherwise assume the datasets sit next to
# the repository (../mvtec/, ../VisA_20220922/, ...). This test is skipped
# entirely when the data and the exported model are not present (see the
# pytestmark below), so CI and a fresh clone without the assets stay green.
_DATA_ROOT = Path(os.environ.get("ADAPTIVEVISION_DATA_ROOT", Path(__file__).resolve().parents[3]))
_MODEL_PATH = (
