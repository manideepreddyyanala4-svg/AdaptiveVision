...
class _FakeModule:
    """Stands in for the ``ollama`` module, recording Client() kwargs."""

    def __init__(self, client: Any) -> None:
        self._client = client
        self.client_kwargs: dict[str, Any] = {}

    def Client(self, **kwargs: Any) -> Any:  # noqa: N802 -- mirrors ollama's API
        self.client_kwargs = kwargs
        return self._client


def _measurement(**overrides: Any) -> DefectMeasurement:
    base: dict[str, Any] = {
        "bbox": (12, 30, 40, 9),
        "area_px2": 210,
        "area_um2": 1312.5,
    ...
    return DefectMeasurement(**base)


def test_engine_binds_configured_host_and_timeout_to_the_client(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """The timeout must be bound to the client itself: a hung local server
    can otherwise stall the caller indefinitely."""
    from adaptivevision import explanation as ollama_engine

    module = _FakeModule(_FakeClient([_valid_payload()]))
    monkeypatch.setattr(ollama_engine, "_try_import_ollama", lambda: module)

    engine = OllamaAdvisoryEngine(host="http://127.0.0.1:11434", timeout_s=0.5)
    engine.generate_report(_evidence())

    assert module.client_kwargs == {"host": "http://127.0.0.1:11434", "timeout": 0.5}


def test_engine_defaults_to_loopback_ollama() -> None:
    """No cloud endpoint may ever be the default: evidence stays on the IPC."""
    assert ollama_defaults_host().startswith("http://127.0.0.1")


def ollama_defaults_host() -> str:
    from adaptivevision.explanation import DEFAULT_HOST

    return DEFAULT_HOST


def test_timeout_falls_back_without_raising(monkeypatch: pytest.MonkeyPatch) -> None:
    """A slow local model must degrade to the deterministic report, never
    propagate an exception into the station."""
    from adaptivevision import explanation as ollama_engine

    client = _FakeClient([TimeoutError("timed out"), TimeoutError("timed out")])
    monkeypatch.setattr(ollama_engine, "_try_import_ollama", lambda: _FakeModule(client))

    report = OllamaAdvisoryEngine(timeout_s=0.5).generate_report(_evidence())

    assert report.is_fallback is True
    assert report.severity == Severity.MAJOR


def test_prompt_fuses_physical_metrology() -> None:
    """The LLM must reason from calibrated microns, not pixels alone."""
    client = _FakeClient([_valid_payload()])
    engine = OllamaAdvisoryEngine(client=client)

    evidence = dataclasses.replace(_evidence(), measurements=(_measurement(),))
    engine.generate_report(evidence)

    prompt = client.calls[0]["prompt"]
    assert "1312.5 um^2" in prompt
    assert "4.44" in prompt
    assert "scratch" in prompt


def test_fallback_classifies_from_measured_morphology() -> None:
    """With metrology present the fallback reports the measured morphology
    rather than a retrieved neighbour's label."""
    from adaptivevision import explanation as ollama_engine

    evidence = dataclasses.replace(_evidence(), measurements=(_measurement(),))
    report = ollama_engine._fallback_report(evidence)

    assert report.defect_classification == "scratch"
    assert "1312.5 um^2" in report.root_cause_hypothesis
    assert report.is_fallback is True


def test_build_evidence_carries_measurements() -> None:
    decision = Decision(verdict=Verdict.FAIL, defects=(Defect(
        defect_class=DefectClass.ANOMALY, severity=Severity.MAJOR),))
    ...
    assert evidence.measurements[0].area_um2 == 1312.5


def test_evidence_measurements_survive_serialization() -> None:
    evidence = dataclasses.replace(_evidence(), measurements=(_measurement(),))
    restored = InspectionEvidence.from_dict(evidence.to_dict())
    assert restored.measurements == evidence.measurements
