...
def test_in_memory_camera_is_camera_driver() -> None:
    assert isinstance(InMemoryCameraDriver(np.zeros((4, 4), dtype=np.uint8)), CameraDriver)


def test_in_memory_camera_open_close_lifecycle() -> None:
    driver = InMemoryCameraDriver(np.zeros((4, 4), dtype=np.uint8))
    assert driver.is_healthy() is False
    driver.open()
    assert driver.is_healthy() is True
    driver.close()
...
def test_in_memory_camera_capture_returns_the_held_image() -> None:
    image = np.full((4, 6, 3), 7, dtype=np.uint8)
    driver = InMemoryCameraDriver(image, camera_id="upload-1")
    driver.open()

    frame = driver.capture(trigger_id="t-1")

    assert frame.camera_id == "upload-1"
    assert frame.trigger_id == "t-1"
    assert frame.image.shape == (4, 6, 3)
    assert int(frame.image[0, 0, 0]) == 7


def test_in_memory_camera_capture_returns_independent_copies() -> None:
    driver = InMemoryCameraDriver(np.zeros((4, 4), dtype=np.uint8))
    driver.open()
    ...
    assert int(driver.capture().image[0, 0]) == 0


def test_in_memory_camera_capture_before_open_raises() -> None:
    driver = InMemoryCameraDriver(np.zeros((4, 4), dtype=np.uint8))
    with pytest.raises(AcquisitionError, match="before open"):
        driver.capture()
