...
#: Lot identifier used when no MES lot feed is wired in. Traceability rows
#: always carry a lot, so downstream lineage queries never see a null.
DEFAULT_LOT_ID = "LOT-ACTIVE"
...
class AnomalyResult(PartialResult):
    ...
    Attributes:
        ...
        defect_measurements: Optional per-region shape measurements extracted
            ...
            wasn't run.
        heatmap_region: Coarse name of where the anomaly signal peaks
            (e.g. ``"upper-right"``), or ``None`` when no localization ran.
        heatmap: The localization map the measurements were taken from.
            Transient working data for the current cycle only: it is
            deliberately not serialized by :meth:`to_dict`, since the
            durable record keeps the archived image and the measurements
            derived from it, not the raw array.
    """
    ...
    heatmap: Image | None = field(default=None, compare=False, repr=False)
...
class InspectionResult:
    ...
    Attributes:
        ...
        heatmap_region: Coarse name of where the anomaly signal peaked, or
            ``None`` when no localization ran.
        lot_id: Production lot this part belongs to, for fab lot lineage.
            Defaults to :data:`DEFAULT_LOT_ID` so a station running without
            an MES lot feed still produces well-formed traceability rows.
    """
    ...
    def from_dict(cls, data: dict[str, Any]) -> Self:
        ...
        return cls(
            ...
            lot_id=data.get("lot_id", DEFAULT_LOT_ID),
            heatmap_region=data.get("heatmap_region"),
        )
...
class InspectionEvidence:
    ...
    Attributes:
        ...
        measurements: Physical defect metrology measured by
            :func:`~adaptivevision.metrology.measure_defects` (area in
            microns squared, aspect ratio, morphology), largest region
            first. Empty when no metrology was run.
    """
    ...
    def from_dict(cls, data: dict[str, Any]) -> Self:
        ...
        return cls(
            ...
            heatmap_region=data.get("heatmap_region"),
            measurements=tuple(
                DefectMeasurement.from_dict(m) for m in data.get("measurements", ())
            ),
        )
