"""Decide stage: fuse the partial results into pass / fail / review.

Deliberately not the AI's job: this fuses the partial results produced by
the inspection inspectors (metrology, anomaly, classical AOI) into a single
verdict with fixed, auditable logic. Defects are never silently dropped, and
a part is only PASS when no inspector raised a defect. The advisory layer
(``explanation.py``) explains this verdict in plain language; it never sets
or overrides it.
...
from adaptivevision.common import AnomalyResult, Defect, PartialResult, Severity, Verdict
...
class DecisionPolicy:
    ...
    def from_recipe(cls, recipe: Recipe) -> DecisionPolicy:
        ...
        ``recipe.decision`` (:class:`~adaptivevision.config.DecisionPolicy`,
        the data-only recipe model) is a data-only description of the
        desired rules; this class is what actually applies them.
        ``max_defects`` and ``fail_severity`` map directly.
        ``review_on_anomaly`` doesn't map onto this policy at all -- it's
        honored by the anomaly *detector* instead (see
        ``ThresholdAnomalyDetector(anomalous_severity=...)``), since it
        decides what severity an anomaly defect carries in the first place,
        not how an already-built defect list is judged.
