...
class InspectionRecord(Base):
    ...
    Attributes:
        ...
        lot_id: Production lot the part belongs to, for fab lot lineage.
        ...
        drift_status: Sensor/illumination drift status in effect at the time
            ...
            in (see ``drift.py``).
        heatmap_region: Coarse region the anomaly signal peaked in. A
            queryable column so a line can trend *where* defects cluster,
            not only how many there were.
    """
    ...
    lot_id: Mapped[str] = mapped_column(String(64), default=DEFAULT_LOT_ID, index=True)
    ...
    heatmap_region: Mapped[str | None] = mapped_column(String(32), nullable=True, index=True)
...
def _from_record(record: InspectionRecord) -> InspectionResult:
    ...
    return InspectionResult(
        ...
        heatmap_region=record.heatmap_region,
    )
...
class LocalImageStore:
    ...
    def archive_image(self, frame_id: str, image: Image) -> str:
        """PNG-encode ``image`` and archive it under ``frame_id``.

        Args:
            frame_id: Unique identifier of the frame (used as the file name).
            image: Grayscale ``(H, W)`` or RGB ``(H, W, 3)`` pixel buffer.

        Returns:
            A stable reference to the archived image.

        Raises:
            ImageStoreError: If the image cannot be encoded or written.
        """
        pixels = np.asarray(image)
        if pixels.dtype != np.uint8:
            # Frames reaching the archive may be float (a preprocessed or
            # rectified buffer); scale to 8-bit for a viewable PNG.
            finite = pixels[np.isfinite(pixels)]
            top = float(finite.max()) if finite.size else 0.0
            scale = 255.0 / top if top > 0 else 0.0
            pixels = np.clip(pixels.astype(np.float64) * scale, 0, 255).astype(np.uint8)
        if pixels.ndim == 3 and pixels.shape[2] == 3:
            # cv2 encodes BGR; frames are carried as RGB.
            pixels = pixels[:, :, ::-1]
        ok, buffer = cv2.imencode(".png", pixels)
        if not ok:
            msg = f"Failed to PNG-encode image {frame_id!r}"
            raise ImageStoreError(msg)
        return self.archive(frame_id, buffer.tobytes())

    def path_for(self, image_id: str) -> Path:
        """Resolve a bare image id to its archived path.

        Args:
            image_id: The frame id an image was archived under. Must be a
                single path segment; anything else is rejected so a request
        ...
        Raises:
            ImageStoreError: If ``image_id`` is unsafe or the image is missing.
        """
        if not image_id or "/" in image_id or "\\" in image_id or image_id in {".", ".."}:
            msg = f"Invalid image id: {image_id!r}"
            raise ImageStoreError(msg)
        return self.resolve(str(self._directory / f"{image_id}.bin"))
